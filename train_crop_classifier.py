...
def train_crop_classifier():
    ...
    crop_path = os.path.join(CROP_CLASSIFIER_ROOT, CROP_CLASSIFIER_FOLDER)
    if not os.path.exists(crop_path):
        print(f"  ⚠️  Skipping — folder nahi mila: {crop_path}")
...
if __name__ == "__main__":
    ...
    # ============================================================
    # COMMAND-LINE OPTIONS — choose what to train
    #   python train_model.py                    → trains everything (default)
    #   python train_model.py --crop-only         → ONLY crop classifier
    #   python train_model.py --diseases-only     → ONLY the 4 disease models
    #   python train_model.py --skip Tomato       → trains everything except Tomato
    # ============================================================
    import sys
    args = sys.argv[1:]

    crop_only     = "--crop-only" in args
    diseases_only = "--diseases-only" in args
    skip_crops    = []
    if "--skip" in args:
        idx = args.index("--skip")
        if idx + 1 < len(args):
            skip_crops = [c.strip() for c in args[idx + 1].split(",")]

    if crop_only:
        print("  ℹ️  Mode: --crop-only (disease models NOT retrained)")
        train_crop_classifier()
    elif diseases_only:
        print("  ℹ️  Mode: --diseases-only (crop classifier NOT retrained)")
        for crop in available:
            if crop not in skip_crops:
                train_crop(crop)
    else:
